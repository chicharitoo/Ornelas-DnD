...
function cardBackHTML(key) {
  ...
  return `<div class="card-back" style="background:${cb.bg};border-color:${d.color}44;box-shadow:inset 0 0 30px ${cb.glow}">
    ${cb.svg}
    <div class="card-back-icon" aria-hidden="true" style="color:${d.textColor}">${d.icon}</div>
...
function stackViewHTML(key) {
  const d = DECKS[key];
  const cb = CARD_BACKS[key];
  return `<div class="stack-view" style="display: block; width: 100%; text-align: center;">
    <div style="display: block; width: 200px; height: 320px; margin: 2rem auto;">
      <div class="stack-container" role="button" tabindex="0" onclick="drawFromStack('${key}')" onkeypress="if(event.key==='Enter'||event.key===' '){drawFromStack('${key}');event.preventDefault()}" aria-label="Draw card" style="margin: 0;">
        ...
        <div class="stack-top" style="background:${cb.bg};border-color:${d.color}66;box-shadow:0 0 40px ${cb.glow}">
          ...
          <div class="stack-top-content">
            ...
            <div class="stack-deck-icon" aria-hidden="true" style="color:${d.textColor}">${d.icon}</div>
            <div class="stack-card-count" style="color:${d.color}">${d.cards.length} cards</div>
          </div>
        </div>
      </div>
      <div class="stack-draw-hint" style="color:${d.color}; display: block; margin-top: 1rem;">tap to draw · </div>
    </div>
    
    <div class="draw-controls" style="margin: 2rem auto; display: flex; flex-wrap: wrap; justify-content: center; gap: 0.5rem;">
  ...
  return `<div class="deck-panel ${key === activeTab ? 'active' : ''}" id="panel-${key}">
    <div class="panel-inner" style="background:${d.bg}44;border-color:${d.color}33">
      <div class="panel-header">
        <div>
          <div class="panel-title" style="color:${d.textColor}"><span aria-hidden="true">${d.icon}</span> ${d.label}</div>
          <div class="deck-count">${d.cards.length} cards · shuffled every view</div>
        </div>
        <div class="panel-controls">
          <div class="view-toggle" style="border-color:${d.color}44">
            <button class="${isStack ? 'active' : ''}" aria-pressed="${isStack ? 'true' : 'false'}" onclick="setView('${key}','stack')" title="Stack view" style="${isStack ? `background:${d.color}33;color:${d.textColor}` : ''}">⬛</button>
            <button class="${!isStack ? 'active' : ''}" aria-pressed="${!isStack ? 'true' : 'false'}" onclick="setView('${key}','grid')" title="Grid view" style="${!isStack ? `background:${d.color}33;color:${d.textColor}` : ''}">⊞</button>
          </div>
          ${!isStack ? `
...
function renderSpreadPanel() {
  if (!storySpread) {
    return `<div class="spread-outer"><div class="spread-box">
      ...
      <div class="format-picker">${FORMATS.map(f => `<button class="format-btn ${f.id === selectedFmt ? 'active' : ''}" aria-pressed="${f.id === selectedFmt ? 'true' : 'false'}" onclick="setFormat('${f.id}')"><span class="fmt-icon" aria-hidden="true">${f.icon}</span><span>${f.label}</span><span class="fmt-sub">${f.sub}</span></button>`).join('')}</div>
...
function render() {
  ...
  }).join('') + `<button role="tab" aria-selected="${activeTab === 'spread'}" aria-controls="panel-spread" class="tab-btn ${activeTab === 'spread' ? 'active' : ''}" style="${activeTab === 'spread' ? 'background:rgba(107,33,168,0.2);border-color:rgba(107,33,168,0.6);color:#e8b84b' : ''}" onclick="switchTab('spread')">✦ Story Spread</button>`;
